...
class TopicKeyword
  ...
  def get_keywords
    @keywords ||=(
      algor = RMMSeg::Algorithm.new(@topic_text)
      keywords = []
      loop do
      ...
      (keywords - STOPWORDS).select{|word|word.chars.count != 1}
    )
  end
  ...
  def _word_frequency(arr)
    all_count = arr.size.to_f
    word_hash = _number_of_times_with_word(arr)
  ...
  # ["a","b","c","c"] => {"a"=>1,"b"=>1,"c"=>2}
  def _number_of_times_with_word(arr)
    word_hash = {}
    arr.each do |str|
